#!/usr/bin/env python3
"""
@package GIALLORO-FIRENZE - MD to PDF builder
@author Padmin D. Curtis (AI Partner OS3.0) for Fabio Cherici
@version 1.1.0 (FlorenceEGI - GIALLORO-FIRENZE)
@date 2026-04-19
@purpose Converte un MD in PDF professionale via headless Chrome.
         Zero dipendenze extra: python-markdown + google-chrome.
"""
import argparse
import subprocess
import sys
...
# Documentazione SSOT vive in EGI-DOC (P0). MAI in /docs del repo.
EGI_DOC = Path("/home/fabio/EGI-DOC/docs/gialloro-firenze")

PRESETS = {
    "audit": {
        "src": EGI_DOC / "audit" / "audit-gialloorofirenze-it.md",
        "title": "Audit tecnico",
        "subtitle": "Analisi SEO &middot; AEO &middot; GEO &middot; UX &middot; Performance &middot; Conformita",
        "recipient": "Edoardo Boccherini &mdash; Giallo Oro Firenze",
        "date": "17 aprile 2026",
        "version": "1.0.0",
        "html_title": "Audit gialloorofirenze.it - FlorenceEGI WebAgency",
    },
    "delivery": {
        "src": EGI_DOC / "delivery" / "delivery-gialloorofirenze-it.md",
        "title": "Documento di Consegna",
        "subtitle": "Cosa consegniamo &middot; Architettura &middot; Posizionamento &middot; Valore di mercato",
        "recipient": "Edoardo Boccherini &mdash; Giallo Oro Firenze",
        "date": "19 aprile 2026",
        "version": "1.0.0",
        "html_title": "Consegna gialloorofirenze.it - FlorenceEGI WebAgency",
    },
}

parser = argparse.ArgumentParser(description="Converte MD in PDF professionale.")
parser.add_argument("preset", nargs="?", default="audit", choices=list(PRESETS.keys()),
                    help="Preset cover (audit | delivery). Default: audit.")
...
if not SRC.exists():
    print(f"[err] sorgente non trovato: {SRC}", file=sys.stderr)
    sys.exit(1)
...
<div class="cover">
  ...
  <div>
    <div class="rule"></div>
    <h1 class="title">{cfg['title']}<br/><span style="color:#92760f">gialloorofirenze.it</span></h1>
    <div class="sub">{cfg['subtitle']}</div>
    <div class="rule"></div>
  </div>
  <div class="meta">
    <strong>Destinatario</strong>: {cfg['recipient']}<br/>
    <strong>Redatto da</strong>: FlorenceEGI WebAgency &mdash; Padmin D. Curtis (AI Partner OS3.0) for Fabio Cherici<br/>
    <strong>Data</strong>: {cfg['date']}<br/>
    <strong>Versione</strong>: {cfg['version']}
  </div>
</div>
"""
